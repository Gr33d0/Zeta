...
async function validateOutput(result, data, id, level) {
  ...
  if (result === data && userStars === level) {
    ...
    console.log("Nivel já completado ou resposta incorreta" + userStars +"Level:" + level);
  }
}

export default function ExecutionResult({ result, data, id, level }) {
  const errText =
    result &&
